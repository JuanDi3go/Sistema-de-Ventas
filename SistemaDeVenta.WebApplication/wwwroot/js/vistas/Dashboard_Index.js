$(document).ready(function () {
    ...
    fetch("/DashBoard/ObtenerResumen").then(response => {
        ...
        if (responseJson.estado) {
            ...
            if (d.ventasSemanaList.length > 0) {
                barchart_labels = d.ventasSemanaList.map((item) => { return item.fecha });
                barchart_data = d.ventasSemanaList.map((item) => { return item.total });
            } else {
                barchart_labels = ["sin Resultados"]
                barchart_data = [0]

            }
            //obtener textos y valores graficos de pie
